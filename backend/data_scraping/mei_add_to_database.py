print("Executing mei_add_to_database.py")
...
from app.models import MonthlyMEI
...
2023    -1.11    -0.91    -0.76    -0.37    -0.06     0.43     0.50     0.51     0.68     0.48     0.91     1.13
2024     0.70     0.68     0.78     0.34     0.12    -0.23    -0.72    -0.73    -0.65    -0.52    -0.68    -0.91
"""


def get_mei_dataframe():
...
df = get_mei_dataframe()

def insert_mei_into_db():
    for _, row in df.iterrows():
        year = int(row['Year'])
        mei_values = {
            f'm{i}': row[str(i)] if not pd.isna(row[str(i)]) else None
            for i in range(1, 13)
        }
        MonthlyMEI.objects.update_or_create(
            year=year,
            defaults=mei_values
        )
